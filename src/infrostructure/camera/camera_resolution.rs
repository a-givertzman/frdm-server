...
impl CameraResolution{
    ...
    pub fn new(parent: impl Into<String>, conf_tree: &mut ConfTree) -> Self {
        ...
        let self_width = self_conf.get_param_value("width").unwrap().as_str().unwrap().parse().unwrap();
        debug!("{}.new | width: {:?}", self_id, self_width);
        let self_height = self_conf.get_param_value("height").unwrap().as_str().unwrap().parse().unwrap();
