...
use crate::infrostructure::arena::{ac_err::AcErr, ac_image::AcImage, bindings::{acBufferGetSizeFilled, acDeviceRequeueBuffer, acImageGetData, acImageGetHeight, acImageGetTimestampNs, acImageGetWidth}};
...
impl AcBuffer {
    ...
    pub fn get_image(&self) -> Result<AcImage, StrErr> {
        ...
        let err = AcErr::from(unsafe { acBufferGetSizeFilled(self.buffer, &mut size_filled) });
        ...
        log::trace!("{}.acquire_images | timestamp (ns): {})", self.name, timestamp_ns);
        ...
        log::trace!("{}.acquire_images | and requeue", self.name);
