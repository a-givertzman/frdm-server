use crate::{
    ...
    }, domain::{Error, Image}, Context, ContextRead, ContextWrite, NormalizedCtx, ResultCtx
};
...
//
//
impl ContextWrite<NormalizedCtx, ResultCtx<Image>> for Context {
    fn write(mut self, value: ResultCtx<Image>) -> Result<Self, Error> {
        self.normalized.result = value;
        Result::Ok(self)
    }
}
impl ContextRead<NormalizedCtx, ResultCtx<Image>> for Context {
