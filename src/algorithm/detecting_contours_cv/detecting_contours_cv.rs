            ...
            Ok(ctx) => {
                ...
                // match imgproc::cvt_color(&frame.mat, &mut gray, imgproc::COLOR_BGR2GRAY, 0) {
                    // Ok(_) => {
                        ...
                        let y_order = 0;
                        match imgproc::sobel(&frame.mat, &mut sobelx, core::CV_8U, x_order, y_order, self.conf.sobel.kernel_size, self.conf.sobel.scale, self.conf.sobel.delta, core::BORDER_DEFAULT) {
                            Ok(_) => {
                                    //
                                    ...
                                    let y_order = 1;
                                match imgproc::sobel(&frame.mat, &mut sobely, core::CV_8U, x_order, y_order, self.conf.sobel.kernel_size, self.conf.sobel.scale, self.conf.sobel.delta, core::BORDER_DEFAULT) {
                                    Ok(_) => {
